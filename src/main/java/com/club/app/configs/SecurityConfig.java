...
public class SecurityConfig {
	...
	SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		...
		http.cors(cors -> {
			...
			auth
					// 로그인 필요한 페이지
					.requestMatchers("/member/update", "/member/detail", "/member/delete", "/member/pwChange",
							"/member/mypage", "/product/add", "/product/detail", "/product/myList", "/product/delete",
							"/product/edit", "/product/addFile", "/product/deleteFile", "/job/create", "/job/create/**",
							"/job/update", "/job/update/**", "/job/delete", "/job/delete/**", "/job/deleteFile",
							"/job/deleteFile/**", "/job/myJobList", "/jobApply/**",

							"/restaurant/create", "/restaurant/create/**",

							"/club/create", "/club/create/**", "/clubSchedule/list", "/clubSchedule/create",
							"/clubSchedule/create/**", "/productChat/**")
					.authenticated()
				...
				.formLogin(login -> {
					login.loginPage("/member/login").usernameParameter("memberId").passwordParameter("memberPw")
							.loginProcessingUrl("/member/login").defaultSuccessUrl("/", true)
							.failureHandler(loginFailHandler).permitAll();
				})

				.rememberMe(remember -> remember.rememberMeParameter("remember-me")
						.tokenValiditySeconds(60 * 60 * 24 * 7).key("myRememberKey"))
