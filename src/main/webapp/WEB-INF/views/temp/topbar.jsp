                                    ...
                                    <c:forEach items="${recentNotifications}" var="dto">
                                        ...
                                        <c:if test="${not empty dto.notificationNum}">
                                            <a class="dropdown-item mb-2" href="/notification/read?notificationNum=${dto.notificationNum}&url=${dto.notificationUrl}"
                                                style="border-radius: 12px; padding: 12px; background-color: ${dto.isRead ? '#f7f3ef' : '#fff8f1'}; border: 1px solid ${dto.isRead ? '#e5ded8' : '#ffcfaa'}; white-space: normal;">
    ...
    (function connectNotificationSocket() {
        ...
        socket.onmessage = function(event) {
            ...
            if (msg.type === "notification") {
                ...
                newNoti.innerHTML = '<div style="font-size: 14px; color: #3f2d20;">' + msg.messageContent + '<span class="badge badge-warning ml-1"> NEW </span></div><div style="font-size: 12px; color: #999; margin-top: 6px;">방금 전</div>';
